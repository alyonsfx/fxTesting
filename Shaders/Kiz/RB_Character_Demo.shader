...
Shader "Rocket Boy/Character/Probe Lighting (Demo)"
{
	Properties
	{
		...
		_FlatTex("Diffuse Texture Blend", Range(0.0, 1.0)) = 1.0
		_DiffusePower("Diffuse Intensity", Range(0.0, 1.0)) = 1.0
		_LightingMask("Lighting Mask", Range(0.0, 1.0)) = 1.0
		...
		_FlatSpec("Specular Texture Blend", Range(0.0, 1.0)) = 1.0
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed _Desat, _SpecPower, _SpecRoll, _DetailPower, _GlowIntensity, _LightingMask, _DiffusePower, _FlatTex, _FlatSpec;
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				main.xyz = lerp(fixed3(0.5f, 0.5f, 0.5f), main.xyz, _FlatTex);
				fixed4 spec = desaturate(tex2D(_SpecTex, i.uv), 1 - _Desat);
				spec.xyz = lerp(fixed3(1, 1, 1), spec.xyz, _FlatSpec);
				//Detail Overlay
				...
				// diffuse shading
				fixed4 c = main;
				fixed dmask = lerp(1, 1 - spec.a, _LightingMask) * _DiffusePower;

				c.xyz = main.xyz * lerp(fixed3(1,1,1), i.diffuse, dmask.xxx);
