...
Shader "Custom/DWD/Sprites/Scrolling Shine Additive"
{
   Properties
   {
   ...
   SubShader
   {
      ...
      Pass
      {
         ...
         half3 ApplyOverlayHalf(half3 base, half3 overlay)
         {
         ...
         half4 frag(fragInput IN): SV_Target
         {
            ...
            #if _SPRITEMODE_MASK
               half4 redColor = _RedColor * c.rrrr;
               half4 greenColor = _GreenColor * c.gggg;
               half4 blueColor = _BlueColor * c.bbbb;
               half mask = 1.0 - saturate(c.r + c.g + c.b);
               half4 noColor = _NoColor * mask.xxxx;

               half redShine = _RedShine * c.r;
               half greenShine = _GreenShine * c.g;
               half blueShine = _BlueShine * c.b;
               half noShine = _NoShine * mask;
            ...
            half s = tex2D(_ShineTex, (IN.coords.xy * _ShineTex_ST.xy) + _ShineTex_ST.zw).r;
            half grad = s * saturate( pow(IN.coords.z, _ShineContrast) * _ShineContrast);

            half overlay = ApplyOverlayHalf(c.rgb, _ShineColor.rgb);
            half3 mix = saturate(lerp(c.rgb, overlay, grad * _ShineColor.a * _ShineIntensityBoost * shineMask));
            mix *= alpha.xxx;

            half levelGrad = baseShineUV.x - (_LevelShineProgress * 2.0 - 1.0);
            levelGrad = saturate(abs((levelGrad * 2.0) - 1.0));

            // saturate levelGrad before feeding into pow() to avoid imaginary number result
            half levelGradAdjusted = saturate(1.0 - levelGrad - _LevelShineScale);
            half level = saturate(pow(levelGradAdjusted, _LevelShineContrast) * _LevelShineBoost) * _LevelShineColor.a * shineMask * s;
            half3 levelShine = _LevelShineColor.rgb * level.xxx;
            mix += levelShine;
