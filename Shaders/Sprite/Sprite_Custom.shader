...
Shader "Sprites/Custom/Vertex Channel Packed"
{
	Properties
	{
		[PerRendererData] _MainTex ("Sprite Texture", 2D) = "white" { }
		[Header(Renderer Color is channel packed)]
		[Header(Red channel multiplies RGB)]
		[Header(Green channel adds to RGB)]
		[Header(Blue channel lerps to Target Color)]
		[Header(Alpha channel multiplies A)]
		[Space(20)]
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 SpriteFrag(v2f IN): SV_Target
			{
				...
				c.rgb += IN.color.g;
				c.a *= IN.color.a;
				return lerp(c, _Target, IN.color.b);
			}
