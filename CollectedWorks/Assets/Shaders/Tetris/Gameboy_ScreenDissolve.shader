Shader "Custom/Mino/Skins/Gameboy Screen Dissolve"
{
	Properties
	{
		[PerRendererData] _MainTex ("Sprite Texture", 2D) = "white" { }
		_Erode ("Errosion", Float) = 0
		_Roundness ("Roundness", Range(0.00, 1.00)) = 1.00
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 SpriteFrag(v2f IN): SV_Target
			{
				...
				screenPos.x = lerp(screenPos.x, screenPos.x * aspectRatio, _Roundness);
				half hCenter = lerp(0.5, 0.5 * aspectRatio, _Roundness);
				half dist = distance(screenPos, half2(hCenter, 0.5));
