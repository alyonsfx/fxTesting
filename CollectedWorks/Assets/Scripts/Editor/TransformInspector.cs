...
{
	static private TransformInspector instance;
	...
	private void DrawRotation()
	{
		...
		GUILayout.BeginHorizontal();
		{
			...
			if (GUILayout.Button(IsolateXButtonContent, EditorStyles.miniButtonMid, TinyButtonWidth))
			{
				mRot.quaternionValue = Quaternion.Euler(mRot.vector3Value.x, 0f, 0f);
			}

			if (GUILayout.Button(IsolateYButtonContent, EditorStyles.miniButtonMid, TinyButtonWidth))
			{
				mRot.quaternionValue = Quaternion.Euler(0f, mRot.vector3Value.y, 0f);
			}

			if (GUILayout.Button(IsolateZButtonContent, EditorStyles.miniButtonRight, TinyButtonWidth))
			{
				mRot.quaternionValue = Quaternion.Euler(0f, 0f, mRot.vector3Value.z);
			}
		}
		GUILayout.EndHorizontal();
	}
	...
	private Vector3 DrawVectorEditor(Vector3 v)
	{
